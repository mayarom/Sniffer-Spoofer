...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # The values passed to .bind() . we are using socket.AF_INET.So it expects a two-tuple: (host, port).
        server_socket.bind((host, port))
        # the server socket listens for connections from clients
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # * Fill in start (3)
            # an infinite while loop is used to loop over blocking calls to client socket.recv()
            ...
            try:
                ...
                # * Fill in start (4)
                # This reads whatever data the client sends and echoes it back using "sendall".
