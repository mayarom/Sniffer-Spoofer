...
def process_request(request: api.CalculatorHeader, server_address: tuple[str, int]) -> tuple[api.CalculatorHeader, int, int, bool, bool, bool]:
    ...
    was_stale = False
    cached = False
    # Check if the data is in the cache, if the requests cache-control
    # is 0 we must not use the cache and request a new response
    ...
    # Request is not in the cache or the response is 'stale' so we need to
    # send a new request to the server and cache the response
...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # the proxy socket listens for connections from clients
        ...
        while True:
            try:
                ...
                # the server accept or complete the connection.
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # reads the data the client sends (the bytes)
